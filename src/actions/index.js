...
import { HOST, POKEMON, POKENAME, POKEID, POKETYPES } from "../constants.js";

export const getPokemons = () => async (dispatch) => {
  try {
    const res = await axios.get(HOST + POKEMON);
...
export const getPokemonByName = (name) => async (dispatch) => {
  try {
    const res = await axios.get(HOST + POKENAME + name);
...
export const getPokemonByID = (id) => async (dispatch) => {
  try {
    const res = await axios.get(HOST + POKEID + id);
...
export const getTypes = () => async (dispatch) => {
  try {
    const res = await axios.get(HOST + POKETYPES);
...
export const createPokemon = (input) => async (dispatch) => {
  try {
    const res = await axios.post(HOST + POKEID, input);
